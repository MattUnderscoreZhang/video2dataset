    ...
    def __call__(self, ffmpeg_stream: FFmpegStream, metadata: Metadata, tmpdir: str) -> Tuple[List[FFmpegStream], List[Metadata], Error]:
        if "scale" in self.resize_mode:
            if self.height > 0:
                ...
                ffmpeg_stream = ffmpeg_stream.filter("scale", self.width, -2)
        if "crop" in self.resize_mode:
            ffmpeg_stream = ffmpeg_stream.filter("crop", w=self.width, h=self.height)
        if "pad" in self.resize_mode:
            ffmpeg_stream = ffmpeg_stream.filter("pad", w=self.width, h=self.height)
